...
class Blockchain {
      ...
      return new Promise(async (resolve, reject) => {
         try {
            ...
            if (height < 0) {
               ...
               obj.height = height + 1;
               let pre = self.chain[self.height];
               obj.previousBlockHash = pre.hash;
   ...
   submitStar(address, message, signature, star) {
      ...
      return new Promise(async (resolve, reject) => {
         try {
            ...
            if (currentTime <= 5 * 60 * 1000 + time) {
               let validBlock = bitcoinMessage.verify(
                  message,
                  address,
                  signature
               );
               if (validBlock) {
                  ...
                  let curBlock = await self._addBlock(newblock);
                  resolve(curBlock);
               } else {
   ...
   getStarsByWalletAddress(address) {
      ...
      return new Promise((resolve, reject) => {
         try {
            self.chain.forEach((el) => {
               let info = el.getBData();
    ...
    * 2. Each Block should check them with the previousBlockHash
    */
   validateChain() {
      let self = this;
      let errorLog = [];
